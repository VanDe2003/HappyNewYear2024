...
function init() {
    // Đặt thời gian chờ trước khi thực hiện hành động
    const delayInMilliseconds = 5000; // Thời gian đặt là 3000 miligiây (tương đương 3 giây)

    // Chờ 3 giây trước khi thực hiện các hành động
    setTimeout(() => {
        // Loại bỏ trạng thái "loading-init"
        const loadingElement = document.querySelector(".loading-init");
        if (loadingElement) {
            loadingElement.remove();
        }

        // Gỡ bỏ lớp "remove" từ appNodes.stageContainer
